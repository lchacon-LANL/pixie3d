...
"""Collection of functions for the processing of Pixie3d output files. These functions are mainly used by the 
   ...
   CAUTION: The functions below are sensitive to the following things:  [NO LONGER A PROBLEM]
   1) Positioning of different variable types inside the h5 tree. Functions affected: pixieload(), load_array() (e.g., the 
   covariant_variables dictionary should be the 3rd variable type) [NO LONGER A PROBLEM: Load h5 file with Julia]
   2) Positioning of different quantities within their variable type dictionary. [NO LONGER A PROBLEM: Load h5 file with Julia]
   ...
   with r in [0,1], theta in [0,2pi]. [NO LONGER A PROBLEM: Initial guesses are now calculated]
...
def intersection_of_horizontal_line_with_flux_surface(psit,psi_min,norm,X0,Z0,X,Z):
    """Return the intersection point of a horizontal line from the magnetic axis with the q_95 surface."""
    # Note: if there are multiple contours for psi = 0.95, then the first contour segment might not cross the horizontal line...
    cs1 = plt.contour(X[:,:,0],Z[:,:,0],(psit[:,:]-psi_min)/(norm-psi_min),levels=[0.98],colors="y")
    v1 = cs1.allsegs[0][0] # Contains the first segment of the contour for psi = 0.95
    x_l = np.asarray(v1[:,0]) # X-values for all vertices of q95 contour
    y_l = np.asarray(v1[:,1]) # Y-values for all vertices of q95 contour

    lbind = np.where(x_l < X0)[0] # left branch indices
    lbt = int(lbind[0]) # top of left branch
    lbb = int(lbind[-1]) # bottom of left branch
    
    # Choose part of flux surface suitable for interpolation
    if Z0 > y_l[0]: # magnetic axis higher than 1st point of flux surface
        list_fs_x = list(x_l[0:lbt-1]) # pick upper branch. Leave two points to avoid endpoint problems
        list_fs_y = list(y_l[0:lbt-1])
    elif Z0 < y_l[0]:
        list_fs_x = list(x_l[lbb+2:]) # pick lower branch
        list_fs_y = list(y_l[lbb+2:])
        list_fs_x.append(x_l[0]) # Append first point to the end to ensure there are no gaps
        list_fs_y.append(y_l[0])
        list_fs_x.reverse()
        list_fs_y.reverse()
    else:
        return x_l[0] # In this case, the intersection value is known without needing to interpolate (typically when Z0 = 0.)

    # Further narrow down the flux surface in case it crosses the line multiple times
    if Z0 > y_l[0]:
        for i,y in enumerate(list_fs_y):
            if Z0 < y:
                imax = i
                break
    elif Z0 < y_l[0]:
    ...
    list_fs_x = list_fs_x[0:imax+1]
    list_fs_y = list_fs_y[0:imax+1]

    # Interpolation of the contour line
    flux_surf_interp = scipy.interpolate.interp1d(list_fs_y,list_fs_x,fill_value="extrapolate")
    Xx = flux_surf_interp(Z0)

    return Xx

def get_point_inside_separatrix(X0,Z0,X,Z,psit_int,r_ma,u_ma,shapedcoords=False):
    """Finds the X-coordinate along a horizontal line from the axis to the separatrix that is just inside the bounds of the separatrix."""
    if shapedcoords==True:
        r_new,u_new = pntCnvInGrid_shaped(X0,Z0,r_ma,u_ma) # (r,theta) coordinates along a horizontal line starting at the magnetic axis
    else:
        r_new,u_new = pntCnvInGrid_simple_toroidal(X0,Z0,X,Z)
    #print("r_new=",r_new," r_ma=",r_ma)
    assert abs(r_new - r_ma) < 1e-4 , f"Calculation of x,z -> r was incorrect."
    assert abs(abs(u_new - u_ma) - (2*np.pi))%(2*np.pi) < 1e-4 , f"Calculation of x,z -> theta was incorrect."
    
    psi_check = psit_int((r_new,u_new)) 
    xmax = np.amax(X)
    xold = X0
    xnew = xold
    r_new = 0.5 # This is the initial guess passed to the shaped-coordinate solver to find r_new,u_new
    u_new = 0.

    while psi_check < 0.97 or psi_check >= 0.99 : # Look for a coordinate that is in between the q97 surface and the separatrix
        xnew = 0.5*(xold + xmax) # Advance along the horizontal line until a suitable coordinate is found
        if shapedcoords==True:
            r_new,u_new = pntCnvInGrid_shaped(xnew,Z0,r_new,u_new) # Use the previous coordinate as a guess for the next coordinate
        else:
            r_new,u_new = pntCnvInGrid_simple_toroidal(xnew,Z0,X,Z)
        psi_check = psit_int((r_new,u_new))
        if psi_check >= 0.99 :
            xmax = xnew # If we advance past the separatrix, set this coordinate as a new maximum and advance a smaller amount next time
        else:
            xold = xnew # Otherwise we continue to advance further along the horizontal line

    Xx = xnew # Return the coordinate that lies inside the desired range
    return Xx

def r_psi_list(t,psit,psi_min,norm,X,Z,shapedcoords=False):
    ...
    assert abs(psit[r_ma,u_ma] - psi_min) < 1e-7 , f"Psi_min calculation was incorrect."
    
    X0 = X[r_ma,u_ma,0] # x,z locations of magnetic axis
    Z0 = Z[r_ma,u_ma,0]
    r_ma_log = CnvNumber2LogicalR(r_ma,X) # r,u logical coordinates of magnetic axis 
    u_ma_log = CnvNumber2LogicalU(u_ma,X)

    psit_normed = (psit[:,:] - psi_min)/(norm - psi_min) # Normalized psit
    assert abs(np.amin(psit_normed[:,:])) < 1e-7 , f"Psi_min calculation was incorrect."
    
    psit_int = Grid_Interpolation_Single_Array(psit_normed,tor=True) # Interpolator for psi_toroidal_average.
    
    Xx = get_point_inside_separatrix(X0,Z0,X,Z,psit_int,r_ma_log,u_ma_log,shapedcoords)

    H_line = np.linspace(X0,Xx,100) # horizontal line that connects the magnetic axis and the point near the separatrix
        
    # convert X,Z0 pairs to logical coordinates in the shaped grid
    r_of_hline = [r_ma_log] # start with first position being the magnetic axis
    u_of_hline = [u_ma_log]
    for x in H_line[1:]:
        if shapedcoords==True:
            r,u = pntCnvInGrid_shaped(x,Z0,r_of_hline[-1],u_of_hline[-1]) # last position is initial guess for new one
        else:
            r,u = pntCnvInGrid_simple_toroidal(x,Z0,X,Z)
        r_of_hline.append(r)
        u_of_hline.append(abs(u) % (2*np.pi))

    # psin values of those points    
    psi_val_list = psit_int((r_of_hline,u_of_hline))
    if min(psi_val_list) != psi_val_list[0]:
        print("Psi not monotonic! time=",t)

    # R(psi) interpolation
    #r_of_psin = RegularGridInterpolator((psi_val_list,), r_of_hline, method='linear', bounds_error=False, fill_value = None)
    r_of_psin = interp1d(psi_val_list,r_of_hline, kind = "linear", fill_value = "extrapolate")
...
def create_r_psi_list(psit,Bpsq_tor,x0,X,Z,rank,shapedcoords=False):
    ...
    #if rank == 0 and norm_list != [] : print("psi at the axis and the x-point have been calculated.")
    R_of_psi = []
    r_ma = []
    u_ma = []
    if t_dim == 0 :
        Rs, rmaxis, umaxis = r_psi_list(0,psit,psi_min_list[0],norm_list[0],X,Z,shapedcoords)
        R_of_psi.append(Rs)
        r_ma.append(rmaxis)
        u_ma.append(umaxis)
    else:
        t_range = psit.shape[2]
        R_of_psi = np.zeros([t_range,101])
        for tind, t in enumerate(range(t_range)):
            Rs, rmaxis, umaxis = r_psi_list(t,psit[:,:,t],psi_min_list[t],norm_list[t],X,Z,shapedcoords)
            assert len(Rs) == 101, f"R(psi) array has incorrect dimensions."
            R_of_psi[tind,:] = Rs
            r_ma.append(rmaxis)
            u_ma.append(umaxis)
...
def CnvNumber2LogicalR(r_num,X):
    """Convert a point of the numerical grid to logical coordinates, i.e., r in [0,1]."""
    r_dim = float(X.shape[0] - 1) # Subtract 1 because of python's indexing from 0
    r_max = X[-1,0,0] - X[0,0,0] # The grid does not necessarily extend to exactly r=1
    r_log = np.asarray(r_num)*r_max/r_dim
    return r_log

def CnvNumber2LogicalU(u_num,X):
    """Convert a point of the numerical grid to logical coordinates, i.e., u in [0,2pi]."""
    u_dim = float(X.shape[1] - 1) # Subtract 1 because of python's indexing from 0
    u_log = (2*np.pi)*(np.asarray(u_num)/u_dim)%(2*np.pi)
    return u_log


def Axes_of_Interpolation(arr,X):    
    ...
    r = np.linspace(0.0,1.0,num=X[0]) 
    theta = np.linspace(0.0,2.0*np.pi,num=X[1])
    phi = np.linspace(0.0,2.*np.pi,num=X[2])
        ...
        try:
            if arr.ndim == 3:
                ...
                if arr.shape[2] != 1:
                    ...
                    arr_int = RegularGridInterpolator((r,theta), arr[:,:,0], method='linear', bounds_error=False, fill_value = 0)
            if arr.ndim == 2:
        ...
        try:
            if arr.ndim == 4:
                ...
                if arr.shape[3] != 1:
                    ...
                    arr_int = RegularGridInterpolator((r,theta,phi), arr[:,:,:,0], method='linear', bounds_error=False, fill_value = 0)
            if arr.ndim == 3:
...
def pntCnvInGrid_simple_toroidal(x_co,z_co,X,Z):
    """Convert an x,y point in the numerical grid. Works only for toroidal computational domain."""
    r_log = np.sqrt((x_co - X[0,0,0])**2 + (z_co - Z[0,0,0])**2)
    u_log = np.arctan2((z_co - Z[0,0,0]),(x_co - X[0,0,0]))%(2.*np.pi)
    return r_log,u_log
...
def pntCnvInGrid_shaped(R,Z,initial_guess_r=0.9,initial_guess_u=4.5):
    ...
    initial_guess=[initial_guess_r,initial_guess_u] # visual inspection is needed to determine initial guess
    ...
    u_log = xcross[1]%(2.*np.pi) # keep theta in [0,2pi) range
    return r_log,u_log
    

def Calculation_of_Units_and_Sizes(X):    
...
def findXpoint(Bpsq_tor,x0):
    ...
    Bpsq_int = Grid_Interpolation_Single_Array(Bpsq_tor[:,:],tor=True) # Interpolator for each time step.
    Xpnt = scipy.optimize.fmin(Bpsq_int,x0,disp=False)
    return Xpnt

def magneticAxisAndXpoint(psit,Bpsq_tor,x0):
    ...
    Xpnt= findXpoint(Bpsq_tor,x0) # location of X-point
...
def Normalization_numbers(psit,Bpsq_tor,x0):
    ...
    norm_list = []
    if t_dim == 0:
        psi_min, norm = magneticAxisAndXpoint(psit[:,:],Bpsq_tor[:,:],x0)
        psi_min_list.append(psi_min)
        norm_list.append(norm)
    else:
        t_range = psit.shape[2]
        for t in range(t_range):
            psi_min, norm = magneticAxisAndXpoint(psit[:,:,t],Bpsq_tor[:,:,t],x0)
            psi_min_list.append(psi_min)
            norm_list.append(norm)
