...
cc      xarr = 0d0
     ...
     .                       ,.false.)
      ...
      call setMGBC(max(0,gpos),neq,nxx,nyy,nzz,igrid,xarr,bcnd,iorder=2)
...
cc      write (*,*) 'x',sqrt(sum(da(:,:,:,3)**2))
cc      write (*,*) 'a_w/o_bc'
cc      do k = kmin,kmax
cc        do j = jmin,jmax
cc          do i = imin,imax
cc            write (*,*) i,j,k,da(i,j,k,3)
cc          enddo
cc        enddo
cc      enddo

cc      call setMGBC(max(0,gpos),neq,nxx,nyy,nzz,igrid,da_cnv,bcnd
cc     .            ,arr_cov=da,icomp=IAX,is_vec=.true.)
      call setMGBC(max(0,gpos),neq,nxx,nyy,nzz,igrid,da,bcnd
     .            ,icomp=IAX,is_cnv=is_cnv,is_vec=.true.,iorder=2)

cc      write (*,*) 'a_w_bc'
cc      do k = kmin-1,kmax+1
cc        do j = jmin-1,jmax+1
cc          do i = imin-1,imax+1
cc            write (*,*) i,j,k,da(i,j,k,3)
cc          enddo
cc        enddo
cc      enddo
...
cc     .                  -abs(v0_cnv(i,j,k,3)))*da(i ,j,k,3)
     ...
     .              -v0_cnv(i,j,k,2)*0.5*(da(i,j,km,2)+da(i,j,k,2))
            ...
            cov(3) =  (flxip-flxim)/dxh(ig)
     .               +(flxjp-flxjm)/dyh(jg)
     .               +(flxkp-flxkm)/dzh(kg)

            do ieq=1,3
              y(neq*(ijk-1)+ieq) = (da(i,j,k,ieq)/dt
     .                  + alpha*(cov(ieq)/jac - etal*veclap(ieq)))*vol
            enddo
...
c     #####################################################################
      ...
      real(8)    :: upwind(3),mul,nabla_vv0(3,3)
...
cc     .                        ,0,0,0)
     ...
     .                     ,alt_eom,vol=.false.)
      ...
      do ieq=1,3
        upwind(ieq) =( v0_cnv(i,j,k,1)*nabla_v(1,ieq)
     .                +v0_cnv(i,j,k,2)*nabla_v(2,ieq)
     .                +v0_cnv(i,j,k,3)*nabla_v(3,ieq))/jac

        upwind(ieq) = upwind(ieq)
...
cc        if (upwind(ieq) /= 0d0) then
cc          write (*,*) ieq,upwind(ieq),ieq,nabla_vv0(:,ieq)
cc        endif
...
c     #####################################################################
     ...
     .                ,tensor_x,tensor_y,tensor_z,vol=.false.)
      iig = 3*(ijk-1) + iisig - 1
      cnv = matmul(a_diag(:,iig+1:iig+3),cnv)
...
ccc     tensor_x_diag
ccc     #############################################################
cc      subroutine tensor_x_diag(i,j,k,nxx,nyy,nzz,igx,igy,igz,alt_eom
cc     .                   ,t11,t12,t13,flag)
ccc     -------------------------------------------------------------
...
ccc     Local variables
cc
cc        integer(4) :: ip,igrid,iig
cc        real(8)    :: jac,jac0,jacp,gsuper(3,3)
cc     .               ,acnv(3),acnvp(3),b0cov(3),b0cnv(3),scalar_prod
cc
ccc     Begin program
cc
cc        igrid = igx
...
cc          acnv = matmul(b_diag(:,iig+1:iig+3),acnv)
cc        else !This part does not contribute due to antisymmetry
cccc          call find_curl_vxb(i,j,k,nxx,nyy,nzz,dv,b0_cnv
cccc     .                      ,acnv(1),acnv(2),acnv(3),0,igrid)
cc          t11 = 0d0
cc          t12 = 0d0
cc          t13 = 0d0
cc          return
...
cc
cccc        b0cov(1) = 0.5*(b0_cov(ip,j,k,1)     +b0_cov(i,j,k,1)     )
cccc        b0cov(2) = 0.5*(b0_cov(ip,j,k,2)/jacp+b0_cov(i,j,k,2)/jac0)*jac
cccc        b0cov(3) = 0.5*(b0_cov(ip,j,k,3)     +b0_cov(i,j,k,3)     )
cc
cc        if (flag /= 0) then
cc          call transformFromCurvToCurv(i,j,k,igx,igy,igz
cc     .                        ,b0cov(1),b0cov(2),b0cov(3)
...
ccc     End program
cc
cc      end subroutine tensor_x_diag
cc
ccc     tensor_y_diag
ccc     #############################################################
cc      subroutine tensor_y_diag(i,j,k,nxx,nyy,nzz,igx,igy,igz,alt_eom
cc     .                   ,t21,t22,t23,flag)
ccc     -------------------------------------------------------------
ccc     Calculates tensor components t21-t23 for EM SI operator
ccc     -------------------------------------------------------------
cc
cc        implicit none
cc
...
ccc     End program
cc
cc      end subroutine tensor_y_diag
cc
ccc     tensor_z_diag
ccc     #############################################################
cc      subroutine tensor_z_diag(i,j,k,nxx,nyy,nzz,igx,igy,igz,alt_eom
cc     .                   ,t31,t32,t33,flag)
ccc     -------------------------------------------------------------
ccc     Calculates tensor components t31-t33 for EM SI operator
ccc     -------------------------------------------------------------
cc
cc        implicit none
cc
...
ccc     End program
cc
cc      end subroutine tensor_z_diag
...
cc            psib = 0d0
cccc            psit = 0d0
cc            psib(1) = laplacian(i,j,k,nxx,nyy,nzz,igrid,igrid,igrid
cc     .                         ,dv(:,:,:,1))
cc            psib(3) = (  dv(i,j+1,k,3)
cc     .               -2.*dv(i,j  ,k,3)
cc     .                  +dv(i,j-1,k,3))/grid_params%dyh(jg)**2
cc            psib = -psib*vol*alpha**2*b0_cnv(i,j,k,2)
cccc            write (*,*) psib
c diag ****
            do ieq=1,3
