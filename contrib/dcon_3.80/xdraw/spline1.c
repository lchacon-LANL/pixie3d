...
void bluf(Float *, int n);
void splfac(Float *, Float *, int );
void blus(Float *, Float *, int , int );
  ...
  if(endmode == 1)
    {
      ...
      fs1[m] = (2*
                  (fs[m-2]+fs[m]-2*fs[m-1])
                 +(fs1[m-1]+fs1[m-2])*(xs[m-1]-xs[m-2])
		 -fs1[m-1]*(xs[m]-xs[m-1])
               )/
               ( xs[m]-xs[m-1]);
    }
...
/*-------- evaluate output values --------*/
  ...
  for (i=1; i<=nqty; i++)
    /*  
      ...
      */
      f[i-1] = fs[j+(i-1)*mmax]*z1*z1*(3-2*z1) + fs[j+1+(i-1)*mmax]*z*z*(3-2*z) +
      d*z*z1*(fs1[j+(i-1)*mmax]*z1-fs1[j+1+(i-1)*mmax]*z);
...
/*-------- evaluate first derivatives --------*/
  ...
  for(i=1; i<=nqty; i++)
    ...
    f1[i] = 6*(fs[j+1,i]-fs[j,i])*z*z1/d +
      ...
      */
    f1[i] = 6*(fs[j+1+(i-1)*mmax]-fs[j+(i-1)*mmax])*z*z1/d +
      fs1[j+(i-1)*mmax]*z1*(3*z1-2)+fs1[j+1+(i-1)*mmax]*z*(3*z-2);
...
/*------ evaluate second derivatives. ------*/
  ...
  for(i=1; i<=nqty; i++)
    ...
    f2[i] = (6*(fs[j+1,i]-fs[j,i])*(z1-z)/d -
	     ...
	     */
    f2[i] = (6*(fs[j+1+(i-1)*mmax]-fs[j+(i-1)*mmax])*(z1-z)/d -
	     fs1[j+(i-1)*mmax]*(6*z1-2)+fs1[j+1+(i-1)*mmax]*(6*z-2))/d;
...
/*-------- evaluate third derivatives ------*/
  ...
  for(i=1; i<=nqty; i++)
    ...
    */
    f3[i]=(12*(fs[j+(i-1)*mmax]-fs[j+1+(i-1)*mmax])/d+
	   6*(fs1[j+(i-1)*mmax]+fs1[j+1+(i-1)*mmax]))/(d*d);
}
  ...
  for(is=1; is<=ns; is++)			/* down sweep */
      ...
      for(i=1; i<=n; i++)
	{
	  for(j=max(1-i,-nl); j<=-1; j++)
	    /* x[i,is] -= a[ind(j,i)]*x[i+j,is];*/
	    x[i-1+(is-1)*n] -= a[ind(j,i)]*x[(i+j-1)+(is-1)*n];
	}
      for(i=n; i>=1; i--)			/* up sweep */
        {
          for(j=1; j<=min(n-i,nu); j++)
            x[i-1+(is-1)*n] -= a[ind(j,i)]*x[(i+j-1)+(is-1)*n];
          x[i-1+(is-1)*n] *= a[ind(0,i)];
        }         
    }
}
