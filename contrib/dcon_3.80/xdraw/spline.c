...
void add_to_spline(int i, float xi, float yi)
{
  ...
  *(x + i)   = xi;
  *(f + i)   = yi;
  if (parameterize) *(t + i) = (float)i;	// parameterize=0 always
  if (i>0 && !parameterize)
    {
      dx = *(x+i) - *(x+i-1);
      if (i==1) dx0 = dx;
...
void fit_spline()
{
  //printf("fit_spline, ns=%d, crude=%d, param=%d\n", ns, crudespline, parameterize);
...
int eval_spline(int i, float z, float *xout, float *yout, int first)
{
  ...
  z1 = F1 - z;
  //if (i==0) printf("eval_spline %d: crude=%d param=%d\n", i, crudespline, parameterize);

  if (!crudespline && !parameterize)		// parameterize is always 0
    {
      dx0 = x[i+1] - x[i];
      *xout = x[i] + z * dx0;
      *yout = evalf(f[i], f[i+1], g[i],g[i+1], z,z1, dx0);
    }
  else if (!crudespline && parameterize)	// n/a: parameterize is 0
    {
      dx0 = t[i+1] - t[i];
  ...
  else						// crudespline=1: x,y from 4-fit
    {
      if (first && ns==3 && i==0)
        {
          ax = ay = FZ;
