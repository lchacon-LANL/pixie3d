...
static float psfontsize = (float)1.25;
...
|   postscript -- initiate a redraw() in postscript mode
-----------------------------------------------------------------------------*/
void postscript1(char *gtitle, char *psname, char *psfont)
{
  char *p;
  p = (char *)dataname;		/* penning had problems with ps_init */
  title = gtitle;
  if (!pstitle) titleheight = 0;
  ps_init(psname, "Times-Roman", p);
  redraw();
  ps_showpage();
  ps_close();
}
...
void ps_init(char *filename, char *fontname, char *data)
{
  ...
  flag = strcmp(filename, "default") == 0 ;
  p = (strcmp(filename, "default") == 0) ? "xdraw" : filename;
  if(flag) sprintf(ps_filename, "%s%c.ps", p, suf + (char) pagenumber);
  else    sprintf(ps_filename, "%s", p);
...
void ps_fill()
{
   fprintf(ps_file, "gsave fill grestore \n");
   verify_count = 0;
...
/*-------------------------------
|  ps_fatline, ps_normalline ps_linewidth (0= REALLY thin
-----------------------------------------*/
void ps_fatline()
{
  int w;
  w =8; /* was 3*/
   if ( psfontfac<(float)1) w = 1;
   fprintf(ps_file, "%d setlinewidth\n", w);
}
void ps_normalline()
{
...
/*------------------------------------
|     ps_greekfont -- initialize fond
----------------------------------------*/
void ps_greekfont(int newfont_height)
{
  font_height = newfont_height;
  fprintf(ps_file,
 	  "/%s findfont %d scalefont setfont\n", greekfont, font_height);
}
...
{
  ly1 = oldwin_dy - ly1;
  if (lineno == -3) ly1=wy0+bsep;
  goodstring(s, length?length: strlen(s));
   if((lang==LATIN) || (length==0) )ps_setlabelfont(axistitle);
    else ps_greekfont(axistitle);
  if(lineno != -2)
  {
  ...
  if (lineno > 0)
    ...
    if(lang==GREEK)ps_font(axistitle);
}
...
/*----------------------------------------------------------------------------
|   ps_vertical_begin -- draw vertical string, centered
|		right: 1 = axis title, -1 = tick labels
-----------------------------------------------------------------------------*/
void ps_vertical_begin(int xpos, int ypos )

{
  ypos = oldwin_dy - ypos;
  xpos = wx0 + bsep + afh;
  fprintf(ps_file, "%s", "%---- Start String Vertically\n");
  fprintf(ps_file, "gsave\n%d %d translate\n90 rotate\n", xpos, ypos);
}

/*----------------------------------------------------------------------------
|   ps_vertical_string -- draw vertical string, centered
|		          relative to current position
|                         state = 0 -begin ; 1 - next; -1 - end
-----------------------------------------------------------------------------*/
void ps_vertical_string( int xpos,int ypos, int state,
			 char *string, int length, int lang)
{
  static int xpos_old =0,ypos_old =0;

  if (!state) /* Begin vertical string */
    {
    ...
    if (state == 1) /* Next part of vertical string */
      {
	fprintf(ps_file, "%s", "%---- NextSubstring Vertically\n");	
	goodstring(string, length?length: strlen(string));
	if((lang==LATIN) || (length==0) )ps_setlabelfont(axistitle);
	else ps_greekfont(axistitle);
	/*
	fprintf(ps_file, "%d %d translate\n", xpos_old, ypos_old);
	
	fprintf(ps_file, "(%s) stringwidth pop 2 div neg 0 rmoveto\n",
		showstring);
		*/
  ...
  else    /* state <= 0 or 1 - end of string*/
    {
      xpos_old = 0;
      ypos_old = 0;
      fprintf(ps_file, "grestore\n");
...
/*----------------------------------------------------------------------------
|   ps_grestore -- restore the graphic context
|		
-----------------------------------------------------------------------------*/
...
void ps_close()
{
  ...
  ps_initialized = 0;
  ps_modeon = 0;
  printf("PostScript file %s written.\n", ps_filename);
}
/*=============================================================================
