...
c-----------------------------------------------------------------------
      ...
      CONTAINS
c-----------------------------------------------------------------------
c     subprogram 1. droot_newton.
c     uses Newton's method to find root of scalar REAL f(z).
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE droot_newton(ff,z,err,it)
      ...
      REAL(r8) :: z_old,f_old,f,dz
c-----------------------------------------------------------------------
c     format statements.
c-----------------------------------------------------------------------
 10   FORMAT(a,i2,a,1p,e9.3,a,e10.3)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c     subprogram 2. droot_mueller.
c     uses Mueller's method to find root of scalar REAL f(z).
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE droot_mueller(ff,zz,err,it)

      REAL(r8) :: ff
      REAL(r8), INTENT(INOUT) :: zz
      REAL(r8), INTENT(OUT) :: err
      INTEGER, INTENT(OUT) :: it

      INTEGER :: i
      REAL(r8) :: q,a,b,c,d,d1,s
      REAL(r8), DIMENSION(0:2) :: f,z
c-----------------------------------------------------------------------
c     format statements.
c-----------------------------------------------------------------------
 10   FORMAT(a,i2,a,1p,e9.3,a,e10.3)
...
c-----------------------------------------------------------------------
      DO
         it=it+1
         err=ABS((z(0)-z(1))/z(0))
         WRITE(*,10)"it = ",it,", err = ",err,", z = ",zz
         IF(err < tol)EXIT
         IF(it > itmax)
     $        CALL program_stop("droot_mueller can't find root.")
         q=(z(0)-z(1))/(z(1)-z(2))
...
c-----------------------------------------------------------------------
      RETURN
      END SUBROUTINE droot_mueller
c-----------------------------------------------------------------------
c     subprogram 3. zroot_newton.
