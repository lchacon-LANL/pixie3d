...
c-----------------------------------------------------------------------
      ...
      REAL(r8), PRIVATE :: a,beta0,p,p0,q,r0,sigma,sigma0,p_pres,p_sig
      TYPE(spline_type), PRIVATE :: spl

      CONTAINS
c-----------------------------------------------------------------------
c     subprogram 1. lar_run.
c     constructs large-aspect-ratio equilibrium.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     declarations.
c-----------------------------------------------------------------------
      ...
      REAL(r8), DIMENSION(:), ALLOCATABLE :: r2
      REAL(r8), DIMENSION(:,:), ALLOCATABLE :: temp

      NAMELIST/lar_input/mtau,ma,r0,a,beta0,q0,p_pres,p_sig,zeroth,
     $     out,bin,m,n
c-----------------------------------------------------------------------
c     formats.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF(out)THEN
         OPEN(UNIT=lar_out_unit,FILE="lar.out",STATUS="unknown")
...
c-----------------------------------------------------------------------
      ...
      xfac=1-x*x
      p=p0*xfac**p_pres
      pp=-2*p_pres*p0*x*xfac**(p_pres-1)
...
c-----------------------------------------------------------------------
      ...
      dy(3)=y(1)*r0/r
      dy(4)=y(5)*(q/r)**2/r
      dy(5)=y(2)*(r/q)**2*(r/r0)*(1-2*(r0*q/y(2))**2*pp)
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
      RETURN
...
c-----------------------------------------------------------------------
      ...
      DO ir=1,mr
         ...
         q=spl%f(8)
         q1=spl%f1(8)/v1
         chi1=twopi*spl%f(1)*r0/(r*v1)
         chi2=chi1*(spl%f1(1)/spl%f(1)-2/r)
         jtheta=-twopi*r0*spl%f1(2)/v1
         singfac=m-n*q
         g11=1/(twopi**2*r0*r)**2
         g22=(twopi*r)**2
         g33=(twopi*r0)**2
         f=(chi1*singfac)**2*g22*g33/(m**2*g33+n**2*g22)
         k=chi1*chi2*g22+q*chi1*(q*chi2+q1*chi1)+2*p1
     $        -chi1*(n*g22+m*q*g33)/(m**2*g33+n**2*g22)
     $        *(n*chi2*g22+m*(q*chi2+q1*chi1)*g33
     $        -2*(singfac*jtheta+n*p1/chi1))
