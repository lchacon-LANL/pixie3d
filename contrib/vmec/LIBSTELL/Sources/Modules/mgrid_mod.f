...
!
      ...
      ALLOCATE (curlabel(dimlens(1)), stat = istat)
      IF (istat .eq. 0) CALL cdf_read(ngrid, vn_coilgrp, 
     1      curlabel(1:dimlens(1)))
...
!
     ...
     3                stat=istat)
            IF (istat .ne. 0) STOP 'ALLOCATION ERROR READING MGRID FILE'
         END IF
